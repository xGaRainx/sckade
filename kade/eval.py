...
def main(args: argparse.Namespace) -> None:
    ...
    metrics = {
        "mean_average_precision":
            scwhat.metrics.mean_average_precision(combined_latent, combined_cell_type),
        "normalized_mutual_info":
            scwhat.metrics.normalized_mutual_info(combined_latent, combined_cell_type),
        "avg_silhouette_width":
            scwhat.metrics.avg_silhouette_width(combined_latent, combined_cell_type),
        "seurat_alignment_score":
            scwhat.metrics.seurat_alignment_score(combined_latent, combined_domain, random_state=0),
        "avg_silhouette_width_batch":
            scwhat.metrics.avg_silhouette_width_batch(combined_latent, combined_domain, combined_cell_type),
        "neighbor_conservation":
            scwhat.metrics.neighbor_conservation(combined_latent, combined_uni, combined_domain)
    }
    if args.paired:
        if len(datasets) != 2:
        ...
        metrics["foscttm"] = np.concatenate(
            scwhat.metrics.foscttm(*[latent[mask] for latent in latents])
        ).mean().item()
    else:
